...
[TestClass]
public class TIMEStartsStartsEntailmentRuleTest : TIMETestOntology
{
    #region Tests
    [TestMethod]
    public async Task ShouldExecuteTIMEStartsStartsEntailmentRule()
    {
        ...
        List<OWLInference> inferences = await TIMEStartsStartsEntailmentRule.ExecuteRuleAsync(ontology);

        Assert.IsNotNull(inferences);
        Assert.HasCount(1, inferences);
        Assert.IsTrue(((OWLObjectPropertyAssertion)inferences[0].Axiom).ObjectPropertyExpression.GetIRI().Equals(RDFVocabulary.TIME.INTERVAL_STARTS));
    ...
    [TestMethod]
    public async Task ShouldExecuteTIMEStartsStartsEntailmentRuleViaReasoner()
    {
        OWLOntology ontology = new OWLOntology(TestOntology);
        ontology.DeclareIntervalFeature(new RDFResource("ex:Feature1"), new TIMEInterval(new RDFResource("ex:Interval1")));
        ontology.DeclareIntervalFeature(new RDFResource("ex:Feature2"), new TIMEInterval(new RDFResource("ex:Interval2")));
        ...
        TIMEReasoner reasoner = new TIMEReasoner().AddRule(TIMEEnums.TIMEReasonerRules.StartsStartsEntailment);
