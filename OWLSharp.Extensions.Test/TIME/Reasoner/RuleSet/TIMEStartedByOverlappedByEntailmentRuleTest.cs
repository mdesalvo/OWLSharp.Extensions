...
[TestClass]
public class TIMEStartedByOverlappedByEntailmentRuleTest : TIMETestOntology
{
    #region Tests
    [TestMethod]
    public async Task ShouldExecuteTIMEStartedByOverlappedByEntailmentRule()
    {
        ...
        List<OWLInference> inferences = await TIMEStartedByOverlappedByEntailmentRule.ExecuteRuleAsync(ontology);

        Assert.IsNotNull(inferences);
        Assert.HasCount(1, inferences);
        Assert.IsTrue(((OWLObjectPropertyAssertion)inferences[0].Axiom).ObjectPropertyExpression.GetIRI().Equals(RDFVocabulary.TIME.INTERVAL_OVERLAPPED_BY));
    ...
    [TestMethod]
    public async Task ShouldExecuteTIMEStartedByOverlappedByEntailmentRuleViaReasoner()
    {
        OWLOntology ontology = new OWLOntology(TestOntology);
        ontology.DeclareIntervalFeature(new RDFResource("ex:Feature1"), new TIMEInterval(new RDFResource("ex:Interval1")));
        ontology.DeclareIntervalFeature(new RDFResource("ex:Feature2"), new TIMEInterval(new RDFResource("ex:Interval2")));
        ...
        TIMEReasoner reasoner = new TIMEReasoner().AddRule(TIMEEnums.TIMEReasonerRules.StartedByOverlappedByEntailment);
