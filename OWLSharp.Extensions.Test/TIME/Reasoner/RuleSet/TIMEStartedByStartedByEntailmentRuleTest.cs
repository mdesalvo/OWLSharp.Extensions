...
[TestClass]
public class TIMEStartedByStartedByEntailmentRuleTest : TIMETestOntology
{
    #region Tests
    [TestMethod]
    public async Task ShouldExecuteTIMEStartedByStartedByEntailmentRule()
    {
        ...
        List<OWLInference> inferences = await TIMEStartedByStartedByEntailmentRule.ExecuteRuleAsync(ontology);

        Assert.IsNotNull(inferences);
        Assert.HasCount(1, inferences);
        Assert.IsTrue(((OWLObjectPropertyAssertion)inferences[0].Axiom).ObjectPropertyExpression.GetIRI().Equals(RDFVocabulary.TIME.INTERVAL_STARTED_BY));
    ...
    [TestMethod]
    public async Task ShouldExecuteTIMEStartedByStartedByEntailmentRuleViaReasoner()
    {
        OWLOntology ontology = new OWLOntology(TestOntology);
        ontology.DeclareIntervalFeature(new RDFResource("ex:Feature1"), new TIMEInterval(new RDFResource("ex:Interval1")));
        ontology.DeclareIntervalFeature(new RDFResource("ex:Feature2"), new TIMEInterval(new RDFResource("ex:Interval2")));
        ...
        TIMEReasoner reasoner = new TIMEReasoner().AddRule(TIMEEnums.TIMEReasonerRules.StartedByStartedByEntailment);
